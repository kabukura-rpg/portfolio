...
const party=document.querySelector('.party-strip');
if(party&&window.KABUKURA_CHARACTERS){party.replaceChildren(...window.KABUKURA_CHARACTERS.map(c=>{const link=document.createElement('a');link.className='party-member';link.href='./characters.html';link.innerHTML=icon('user');link.append(document.createTextNode(c.name));return link;}));}
